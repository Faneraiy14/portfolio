...
Ядро власної ОС з нуля на C (не дистрибутив Linux) — завантаження через
GRUB/multiboot, переривання, сторінкова пам'ять, власна файлова система,
GUI робочий стіл (менеджер вікон, панель задач, меню "Пуск"), повний
TCP/IP-стек (DHCP/DNS/HTTP-клієнт), власний пакетний менеджер і сервер
пакетів, текстовий веб-браузер з нуля, GPT-інсталятор, що виділяє розділ
фіксованого розміру й ніколи не виходить за його межі, і UHCI USB-драйвер.
Dual-boot на реальному ноутбуці, не лише у VM. Частина модулів ядра тепер
написана прямо на NyxilumLang (див. вище) і компілюється в нативний x86
власним бекендом компілятора — без VM під час виконання, справжній
машинний код прямо в бінарнику ядра. Вихідний код приватний.
